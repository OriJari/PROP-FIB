...
public class DriverCollaborativeFiltering {
    ...
    public static void testrecommend(){
        ...
        System.out.println("UserID del dominio.controladores.clases.atribut.user que queremos la recomendacion:");
        int userID = sc.nextInt();
        Map<Integer, Float> recommendation = CF.recommend(userID);
        for(Map.Entry<Integer, Float> entry: recommendation.entrySet()){
            System.out.println("Expected rating for dominio.controladores.clases.atribut.item " + entry.getKey() + ": " + entry.getValue());
        }
    }
